...
export default function MyDogTop() {
  ...
  const [noResults, setNoResults] = useState(false);  // 変更1: 該当結果がない場合の状態
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {/* 変更1: 該当結果がない場合のメッセージ表示 */}
      {noResults && <p className="text-red-500 mt-4">該当がありません。別の条件で検索してね。</p>}

      <div className="flex flex-wrap gap-6 justify-center mt-6">
        {results.map((dog) => (
          <div key={dog.id} className="card bg-base-100 w-64 shadow-xl p-4">
            <figure>
              <img src={dog.image} alt={dog.breed_jp} className="rounded-xl" />
            </figure>
            <div className="card-body">
              {/* 変更2: 見出しの色をオレンジに変更 */}
              <h2 className="text-lg font-semibold mb-2">{dog.breed_jp}</h2>
              <div className="space-y-1">
              <p className="text-sm"><span className="font-bold text-orange-500">特徴:</span> {dog.features}</p>
              <p className="text-sm"><span className="font-bold text-orange-500 text-sm">性格:</span> {dog.personality}</p>
              <p className="text-sm"><span className="font-bold text-orange-500 text-sm">大きさ:</span> {dog.size}</p>
              <p className="text-sm"><span className="font-bold text-orange-500 text-sm">色:</span> {dog.color}</p>
